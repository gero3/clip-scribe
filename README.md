...
- Use `Xenova/whisper-small` by default, with faster and experimental large turbo options available.
...
- `small` is the best practical default for browser use. `onnx-community/whisper-large-v3-turbo` is available as an experimental larger option, but it can still be slow or memory-heavy in browser WASM.
